...
## Как построено (правки после ревью)

План исполнен полностью, но код в трёх местах отличается от написанного ниже —
это результат ревью, а не отклонение. Источник истины — код.

- **Task 5, панель звонка.** В плане декоративные кнопки — `<button aria-hidden="true"
  tabindex="-1">`. Так нельзя: `tabindex="-1"` убирает Tab, но `<button>` остаётся
  фокусируемой мышью, и клик уводит фокус в поддерево, скрытое от дерева доступности
  (нарушение WAI-ARIA, правило axe-core `aria-hidden-focus`). Итог: декоративные
  иконки — `<span aria-hidden="true">`, обработчиков на них и не было.
- **Task 4, индикатор микрофона.** Добавлен `role="img"` к `aria-label` — голый
  `<span>` не имеет роли, поддерживающей доступное имя. Скрывается целиком при
  `state === 'connecting'`.
- **Task 8, компоновка.** Полная сетка 2×2 занимает ~428px и выталкивала варианты
  ответов за пределы экрана ноутбука в каждом раунде. У `VideoCallStage`/`VideoTile`
  появился проп `dense`: на экране вопроса — один ряд из 4 компактных плиток
  (лента участников), в лобби осталась сетка 2×2 как в макете.
